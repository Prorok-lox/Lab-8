...
def task1():
    src = cv2.imread('images/variant-3.jpeg')
...
def task3():
    ...
    while True:
        ...
        if len(contours) > 0:
            c = max(contours, key=cv2.contourArea)
            x, y, w, h = cv2.boundingRect(c)
            if center_rect[0] <= x and x + w <= center_rect[2] and center_rect[1] <= y and y + h <= center_rect[3]:
                cv2.rectangle(frame, (center_rect[0], center_rect[1]), (center_rect[2], center_rect[3]), (0, 255, 0), 2)
            else:
                cv2.rectangle(frame, (center_rect[0], center_rect[1]), (center_rect[2], center_rect[3]), (0, 0, 255), 2)
